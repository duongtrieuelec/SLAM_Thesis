...
<p align="center">
<a href="https://youtu.be/PXG_6LkbtgY"> <img src="Doc/ORB-SLAM2_AR_demo.png" width="640"> </a>
</p>



<p align="center">
<img src="Doc/ORB-SLAM2.png" width="640">
</p>



<p align="center">
<a href="https://youtu.be/L1HEL4kLJ3g"> <img src="Doc/ORB-SLAM2_video.png" width="640"> </a>
</p>
...
```
git clone https://gitlab.inria.fr/tangram/ORB-SLAM2 ORB-SLAM2 --recursive
```
...
ORB-SLAM2 includes a map viewer, an image viewer and a AR viewer.
...
Example to run ORB-SLAM2 on the scene *Table*:

```
./ORB-SLAM2 ../Vocabulary/ORBvoc.txt ../Cameras/MI9T_640x360_0.6.yaml ../Data/sink_21/frames/ ../Data/detections_yolov5_sink_21.json null points+objects sink
```


Example to run ORB-SLAM2 on the scene *Sink*:
...
Example to run ORB-SLAM2 on the scene *fr2/desk*:
```
./ORB-SLAM2 ../Vocabulary/ORBvoc.txt ../Cameras/TUM2.yaml ../Data/rgbd_dataset_freiburg2_desk/rgb.txt ../Data/detections_yolov5_tum_rgbd_fr2_desk.json null points+objects fr2
```

## Live mode

It is possible to run ORB-SLAM2 live using a webcam and YOLOv5. Trained weights are available at: [https://dorel.univ-lorraine.fr/dataset.xhtml?persistentId=doi%3A10.12763%2F2CZWJP](https://dorel.univ-lorraine.fr/dataset.xhtml?persistentId=doi%3A10.12763%2F2CZWJP)
...
```
./ORB-SLAM2 ../Vocabulary/ORBvoc.txt ../Cameras/TUM2.yaml webcam_X yolov5_weights.onnx ../Data/ignore_statues_parts.txt points+objects output_folder
```
...
In ORB-SLAM2, YOLOv5 expects images of size 320 x 320. Original size (640 x 640) can be used by modifying ```src/ImageDetections.cc:108```.
...
Example to run ORB-SLAM2 in **Localization** mode on an existing map of the scene *Table* (pre-built from the image sequence *table_6*):

```
./ORB-SLAM2_localization ../Vocabulary/ORBvoc.txt ../Cameras/MI9T_640x360_0.6.yaml ../Data/table_9/frames/  ../Data/detections_yolov5_table_9.json null ../Data/table_6_map/map_table.yaml points+objects table_reloc 0
```

Example to run ORB-SLAM2 in **Localization** mode on an existing map of the scene *Sink* (pre-built from the image sequence *sink_13*):
```
./ORB-SLAM2_localization ../Vocabulary/ORBvoc.txt ../Cameras/MI9T_640x360_0.6.yaml ../Data/sink_18/frames/  ../Data/detections_yolov5_sink_18.json null ../Data/sink_13_map/map_sink.yaml points+objects sink_reloc 0
```
...
You can build a docker image containing all you need to run ORB-SLAM2 using ```Dockerfile```.
...
Build the image:
```
docker build -t ORB-SLAM2 .
```

Run the docker:
```
sudo xhost +local:root && docker run --gpus all --rm -e DISPLAY=$DISPLAY -v path/to/folder/with/trained_yolov5_in_onnx:/data/ -v /tmp/.X11-unix:/tmp/.X11-unix -v /dev:/dev:ro --device=/dev/video4:/dev/video4 -it ORB-SLAM2 
```
...
ORB-SLAM2 executables are built in the folder ```/opt/ORB-SLAM2/bin```.
...
ORB-SLAM2 is released under a GPLv3 license. The code is based on [ORB-SLAM2](https://github.com/raulmur/ORB_SLAM2).
