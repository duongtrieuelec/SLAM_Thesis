...
int main(int argc, char **argv) {
  ...
  std::cout << "Loading detections from: " << detections_file << std::flush;
  ...
  if (extension == "onnx") { // load network
    // Check if it's a segmentation model (contains "-seg" in filename)
    if (detections_file.find("-seg") != std::string::npos ||
        detections_file.find("_seg") != std::string::npos) {
      std::cout << " (YOLOv8-seg ONNX model)..." << std::flush;
      detector = std::make_shared<ORB_SLAM2::SegmentationDetector>(
          detections_file, classes_to_ignore);
      use_segmentation = true;
    } else {
      std::cout << " (ONNX model)..." << std::flush;
    ...
    std::cout << " (JSON file)..." << std::flush;
    detector = std::make_shared<ORB_SLAM2::DetectionsFromFile>(
        detections_file, classes_to_ignore);
    detect_from_file = true;
    std::cout << " done." << std::endl;
  } else {
    std::cout << "\nInvalid detection file. It should be .json or .onnx\n"
  ...
  // Load images
  std::cout << "Loading image list..." << std::flush;
  cv::VideoCapture cap;
  ...
  if (!use_webcam) {
    ...
    std::cout << " from " << strFile << std::flush;
    LoadImages(strFile, vstrImageFilenames, vTimestamps);
    nImages = vstrImageFilenames.size();
    std::cout << " done. (" << nImages << " images)" << std::endl;
  } else {
    std::cout << std::endl;
  ...
  // Create system
  std::cout << "Creating SLAM system (loading vocabulary, this may take 30-60 "
               "seconds)..."
            << std::endl;
  ...
  int DETECTION_INTERVAL = 5; // Detect every 5 frames to prevent blocking
  ...
  while (1) {
    ...
    if (im.empty()) {
      cerr << endl << "Failed to load image: " << filename << endl;
      return 1;
    }
    ...
    if (detector) {
      if (detect_from_file) {
        detections = detector->detect(filename); // from JSON file
      } else {
        // Interval logic for ONNX to improve performance
        if (ni % DETECTION_INTERVAL == 0) {
          detections = detector->detect(im); // from ONNX neural network
    ...
    cv::Mat m;
    if (use_segmentation && detector) {
      cv::Mat person_mask = detector->getPersonMask();
      m = SLAM.TrackMonocular(im, tframe, detections, false, person_mask);
    } else {
