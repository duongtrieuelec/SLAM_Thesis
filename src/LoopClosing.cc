...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF) {
  ...
  // tree
  {
    ...
    if (!mbStopGBA) {
      ...
      // Get Map Mutex
      unique_lock<mutex> lock(mpMap->mMutexMapUpdate);
      std::cout << "loop closure take mutex map update" << std::endl;

      // Correct keyframes starting at map first keyframe
      list<KeyFrame *> lpKFtoCheck(mpMap->mvpKeyFrameOrigins.begin(),
                                   mpMap->mvpKeyFrameOrigins.end());
      std::unordered_set<KeyFrame *> checkedKF;

      while (!lpKFtoCheck.empty()) {
        KeyFrame *pKF = lpKFtoCheck.front();
        if (pKF)
          std::cout << "Correcting KF : " << pKF->mnId << std::endl;
        else
          std::cout << "Correcting KF : null" << std::endl;
        const set<KeyFrame *> sChilds = pKF->GetChilds();
        cv::Mat Twc = pKF->GetPoseInverse();
        checkedKF.insert(pKF);
        for (set<KeyFrame *>::const_iterator sit = sChilds.begin();
          ...
          if (checkedKF.find(pChild) == checkedKF.end())
      ...
      for (size_t i = 0; i < vpMPs.size(); i++) {
        ...
        if (pMP)
          std::cout << "Correcting MP : " << pMP->mnId << std::endl;
        else
          std::cout << "Correcting MP : null" << std::endl;
      ...
      // Log all map objects after GBA correction
      const std::vector<MapObject *> objects = mpMap->GetAllMapObjects();
      std::cout << "[POST-GBA] Map Objects (" << objects.size()
                << " total):" << std::endl;
      for (auto *obj : objects) {
        if (obj && obj->GetTrack()) {
          int track_id = obj->GetTrack()->GetId();
          int cat_id = obj->GetTrack()->GetCategoryId();
          Eigen::Vector3d center = obj->GetEllipsoid().GetCenter();
          std::cout << "  [POST-GBA] ID=" << track_id << " Cat=" << cat_id
