...
void Tracking::SetARViewer(ARViewer *pARViewer) { mpARViewer = pARViewer; }
...
// GrabImageRGBD with object detections
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD,
                                const double &timestamp,
                                const std::vector<Detection::Ptr> &detections) {
  current_frame_idx_ =
      (current_frame_idx_ + 1) % (std::numeric_limits<size_t>::max() - 1);
  mImGray = imRGB;
  ...
  for (auto det : current_frame_detections_) {
    ...
    // Filter keypoints on person (category_id = 0 in COCO)
    if (det->category_id == 0 && det->score > 0.5) {
      person_bboxes.push_back(det->bbox);
    }
  }

  // Filter out keypoints on person
  if (!person_bboxes.empty() && mState == OK) {
    mCurrentFrame.FilterKeypointsInBboxes(person_bboxes);
  }
...
Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp,
  ...
  // Collect bounding boxes of "person" detections (category_id = 0) for dynamic
  // object filtering
  std::vector<Eigen::Vector4d> person_bboxes;
  for (auto det : current_frame_detections_) {
    // Lower threshold to capture objects with lower confidence scores
    if (det->score > 0.2) {
      // if (det->category_id != 73 && det->score > 0.5 ||  det->score > 0.7) {
      // // for table scene to ignore book on the nappe
      current_frame_good_detections_.push_back(det);
    }
    // Filter keypoints on person (category_id = 0 in COCO)
    if (det->category_id == 0 && det->score > 0.5) {
  ...
  // Filter out keypoints on person to improve SLAM with moving people
  // Only filter after map is initialized - we need all keypoints during
  // initialization
  if (!person_bboxes.empty() && mState == OK) {
    mCurrentFrame.FilterKeypointsInBboxes(person_bboxes);
  }
  ...
  if (force_relocalize) {
    ...
    std::cout << "Start Tracking Frame " << mCurrentFrame.mnId << std::endl;
    Track();
...
bool Tracking::RelocalizationFromObjects(bool use_points) {
  ...
  std::cout << "Nb detections / ellipsoids = " << bboxes.size() << " / "
            << ellipsoids.size() << "\n";

  auto [best_index, poses, scores, used_pairs, used_inliers] =
      ORB_SLAM2::solveP3P_ransac(ellipsoids, ellipsoids_categories, bboxes,
                                 bboxes_categories, K_, 0.2);

  if (best_index == -1 || poses.size() == 0 ||
      scores[best_index] > bboxes.size())
  ...
  for (int i = 0; i < indices.size() && scores[indices[i]] < bboxes.size();
       ++i) {
    ...
    // Match keypoints from the current frame to map points using the estimated
    // pose
    auto map_points = mpMap->GetAllMapPoints();
    ORBmatcher matcher(0.9, true);
    int nb_matches =
        matcher.SearchByProjectionAllPoints(mCurrentFrame, map_points, 10, 100);
    ...
    if (nb_matches > 10) // the pose is considered as good, we can stop
    {
      ...
      mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.mTcw);
      // mnLastRelocFrameId = mCurrentFrame.mnId; // causes problems to resume
      // tracking
      return true;
    }
  }
  ...
  mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.mTcw);
  // mnLastRelocFrameId = mCurrentFrame.mnId; // cause problems to resume
  // tracking
...
// } // namespace ORB_SLAM2 (Moved to end)

void Tracking::TrackObjects(const cv::Mat &im) {
  // Check if state is OK before proceeding (logic from GrabImageMonocular)
  // The previous block was inside else { ... if(mState == OK) }
  // We assume the caller handles the top-level else, but we check mState == OK
  // here.
  ...
  if (!objectTracks_.empty()) {
    for (auto tr : objectTracks_) {
      // 0: ONLY_2D, 1: INITIALIZED, 2: IN_MAP
      std::cout << "  Track ID: " << tr->GetId()
                << " Status: " << (int)tr->GetStatus()
                << " Obs: " << tr->GetNbObservations() << "\n";
    }
  }
  ...
  if (mState == Tracking::OK) {
    ...
    if (current_frame_good_detections_.size() != 0) {
      ...
      for (auto tr : objectTracks_) {
        ...
        if (obj) {
          Eigen::Matrix3d K =
              cvToEigenMatrix<double, float, 3, 3>(mCurrentFrame.mK);
          Matrix34d P = K * Rt;
          Ellipse ell = obj->GetEllipsoid().project(P);
          bb = ell.ComputeBbox();
          if (bboxes_intersection(bb, img_bbox) > 0)
            is_visible = true;
          if (is_visible) {
            proj_bboxes[tr] = bb;
          }
        ...
        if (!is_visible &&
            tr->GetLastObsFrameId() + TIME_DIFF_THRESH < current_frame_idx_)
          continue;
        if (tr->GetStatus() == ObjectTrackStatus::IN_MAP && !is_visible) {
          continue;
        }
        possible_tracks.push_back(tr);
      }
      ...
      for (int i = 0; i < mCurrentFrame.N; ++i) {
        MapPoint *pMP = mCurrentFrame.mvpMapPoints[i];
        if (pMP && !mCurrentFrame.mvbOutlier[i]) {
          cv::Point2f &pt = mCurrentFrame.mvKeysUn[i].pt;
          for (size_t i = 0; i < current_frame_good_detections_.size(); ++i) {
            if (is_inside_bbox(pt.x, pt.y,
                               current_frame_good_detections_[i]->bbox)) {
              assoc_map_points[i].insert(pMP);
            }
          }
        }
      }
      for (size_t i = 0; i < current_frame_good_detections_.size(); ++i) {
        int n = assoc_map_points[i].size();
        for (auto corresp_map_point : assoc_map_points[i]) {
          if (corresp_map_point->IsInKeyFrame(mpLastKeyFrame)) {
            assoc_map_points[i].insert(corresp_map_point);
          }
        }
      }
