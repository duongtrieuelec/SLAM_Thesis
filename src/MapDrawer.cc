...
void MapDrawer::DrawMapObjectsPoints(double size) {
  ...
  for (auto *obj : objects) {
    ...
    glColor3f(static_cast<double>(c(2)) / 255, static_cast<double>(c(1)) / 255,
...
void MapDrawer::DrawMapObjects() {
  ...
  for (auto *obj : objects) {
    ...
    if (!display_3d_bbox_) {
      ...
      pts << -axes[0], -axes[1], -axes[2], axes[0], -axes[1], -axes[2], axes[0],
          axes[1], -axes[2], -axes[0], axes[1], -axes[2], -axes[0], -axes[1],
          axes[2], axes[0], -axes[1], axes[2], axes[0], axes[1], axes[2],
    ...
    // Draw label at object center - print to console for verification
    {
      int cat_id = obj->GetTrack()->GetCategoryId();
      int track_id = obj->GetTrack()->GetId();
      Eigen::Vector3d center = ell.GetCenter();

      // Track which objects we've already printed (to avoid spam)
      static std::unordered_set<int> printed_objects;
      if (printed_objects.find(track_id) == printed_objects.end()) {
        printed_objects.insert(track_id);

        // Get class name based on COCO category ID
        ...
        std::cout << "[MAP OBJECT] ID=" << track_id << " Category=" << cat_id
                  << " (" << label << ")"
