...
Viewer::Viewer(System *pSystem, FrameDrawer *pFrameDrawer,
    ...
    : mpSystem(pSystem), mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer),
      ...
      mbStopped(true), mbStopRequested(false), mbPaused(false) {
...
void Viewer::Run() {
  ...
  pangolin::Var<float> menuPointsSize("menu.Points Size", 1.61, 1e-1, 1e1,
                                      true);
  pangolin::Var<float> menuObjectsPointsSize("menu.Obj-Points Size", 1.0, 1e-1,
                                             1e1, true);
  ...
  while (1) {
    try {
      ...
      if (menuDistEstim)
        mpMapDrawer->DrawDistanceEstimation(mpTracker->GetCurrentMeanDepth(),
                                            mpTracker->mCurrentFrame.mTcw);
      ...
      std::cerr << "Viewer Exception: " << e.what() << std::endl;
    } catch (...) {
      std::cerr << "Viewer crashed with unknown exception!" << std::endl;
    }
  }
...
bool Viewer::isPaused() { return mbPaused; }
