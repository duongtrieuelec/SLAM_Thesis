...
Frame::Frame(const Frame &frame)
    : mpORBvocabulary(frame.mpORBvocabulary),
      ...
      mvFeatureWeights(frame.mvFeatureWeights) {
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  mvFeatureWeights = vector<float>(N, 1.0f);
...
void Frame::FilterKeypointsInBboxes(
    const std::vector<Eigen::Vector4d> &bboxes) {
  if (bboxes.empty() || mvKeys.empty())
    return;

  std::vector<cv::KeyPoint> filteredKeys;
  std::vector<cv::KeyPoint> filteredKeysUn;
  cv::Mat filteredDescriptors;
  std::vector<float> filteredURight;
  ...
  for (int i = 0; i < N; i++) {
    ...
    // Check if keypoint is inside any bounding box
    for (const auto &bbox : bboxes) {
      // bbox format: [x1, y1, x2, y2]
      if (kp.pt.x >= bbox[0] && kp.pt.x <= bbox[2] && kp.pt.y >= bbox[1] &&
          kp.pt.y <= bbox[3]) {
        insideBbox = true;
        break;
      }
    ...
    // Keep keypoint only if NOT inside any bbox
    if (!insideBbox) {
      filteredKeys.push_back(kp);
      if (!mvKeysUn.empty())
        filteredKeysUn.push_back(mvKeysUn[i]);
      if (!mDescriptors.empty())
        filteredDescriptors.push_back(mDescriptors.row(i));
      if (!mvuRight.empty())
        filteredURight.push_back(mvuRight[i]);
      if (!mvDepth.empty())
        filteredDepth.push_back(mvDepth[i]);
    }
  }

  // Update frame data
  int filteredCount = N - static_cast<int>(filteredKeys.size());
  mvKeys = filteredKeys;
  mvKeysUn = filteredKeysUn;
  mDescriptors = filteredDescriptors.clone();
  mvuRight = filteredURight;
  mvDepth = filteredDepth;
  N = mvKeys.size();

  // Reinitialize associated data structures
  mvpMapPoints = std::vector<MapPoint *>(N, static_cast<MapPoint *>(NULL));
  mvbOutlier = std::vector<bool>(N, false);

  // Reassign features to grid
  for (unsigned int i = 0; i < FRAME_GRID_COLS; i++)
    for (unsigned int j = 0; j < FRAME_GRID_ROWS; j++)
      mGrid[i][j].clear();
  AssignFeaturesToGrid();
  ...
  // Debug output
  if (filteredCount > 0)
    std::cout << "Filtered " << filteredCount
              << " keypoints on dynamic objects\n";
}

void Frame::FilterKeypointsWithMask(const cv::Mat &mask) {
  if (mask.empty() || mvKeys.empty())
    return;
  ...
  for (int i = 0; i < N; i++) {
    ...
    bool insideMask = (mask.at<uchar>(y, x) > 0);

    // Keep keypoint only if NOT inside the mask
    if (!insideMask) {
      filteredKeys.push_back(kp);
      if (!mvKeysUn.empty())
        filteredKeysUn.push_back(mvKeysUn[i]);
...
void Frame::AssignFeatureWeights(const cv::Mat &weight_mask) {
  if (weight_mask.empty())
    return;

  if (weight_mask.type() != CV_32F) {
    std::cerr << "AssignFeatureWeights: mask must be CV_32F" << std::endl;
    return;
  }
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (u >= 0 && u < weight_mask.cols && v >= 0 && v < weight_mask.rows) {
      float w = weight_mask.at<float>(v, u);
      if (w > 1.0f) {
        mvFeatureWeights[i] = w;
      }
    }
  }
}
