...
cv::Mat FrameDrawer::DrawFrame() {
  ...
  if (state == Tracking::NOT_INITIALIZED) // INITIALIZING
  {
    for (unsigned int i = 0; i < vMatches.size(); i++) {
      if (vMatches[i] >= 0 && i < vIniKeys.size() &&
          vMatches[i] < vCurrentKeys.size()) {
...
void FrameDrawer::Update(Tracking *pTracker) {
  ...
  if (pTracker->mLastProcessedState == Tracking::NOT_INITIALIZED) {
    ...
    mvIniMatches = pTracker->mvIniMatches;
             ...
             pTracker->mLastProcessedState == Tracking::LOST) {
    for (int i = 0; i < N; i++) {
  ...
  for (auto det : detections) {
    detections_widgets_.push_back(
        DetectionWidget(det->bbox, 0, det->category_id, det->score,
                        cv::Scalar(0, 0, 0), 2, false)); // 1.5
  }
  ...
  for (auto tr : tracks) {
    if (current_frame_id == tr->GetLastObsFrameId()) {
      ...
      detections_widgets_.push_back(
          DetectionWidget(bb, tr->GetId(), tr->GetCategoryId(),
                          tr->GetLastObsScore(), tr->GetColor(), 3, true));
    }
  }
  ...
  if (cv_Rt.rows == 4 && cv_Rt.cols == 4) {
    ...
    for (auto &tr : tracks) {
      ...
      // if (tr->GetCategoryId() == 1 || tr->GetCategoryId() == 2 ||
      // tr->GetCategoryId() == 3 || tr->GetCategoryId() == 9
      // ||tr->GetCategoryId() == 10 || tr->GetCategoryId() == 11) continue;

      const auto *obj = tr->GetMapObject();
      if (obj) {
        auto proj = obj->GetEllipsoid().project(P);
        object_projections_widgets_.push_back(ObjectProjectionWidget(
            proj, tr->GetId(), tr->GetCategoryId(), tr->GetColor(),
            tr->GetStatus() == ObjectTrackStatus::IN_MAP, tr->unc_));
      }
      // }
    }
  }
...
cv::Mat FrameDrawer::DrawDetections(cv::Mat img) {
  std::vector<DetectionWidget, Eigen::aligned_allocator<DetectionWidget>>
      detections;
  {
    unique_lock<mutex> lock(mMutex);
  ...
  for (auto d : detections) {
    ...
    if (std::isnan(bb[0]) || std::isnan(bb[1]) || std::isnan(bb[2]) ||
        std::isnan(bb[3]))
      continue;

    cv::Point2i tl(std::max(0, (int)bb[0]), std::max(0, (int)bb[1]));
    cv::Point2i br(std::min(img.cols - 1, (int)bb[2]),
                   std::min(img.rows - 1, (int)bb[3]));
    ...
    cv::rectangle(img, tl, br, color, d.thickness);
    if (d.display_info) {
      std::stringstream ss;
      ss << std::fixed << std::setprecision(2) << d.score;
      // cv::putText(img, std::to_string(d.id) + "(" +
      // std::to_string(d.category_id) + ") | " + ss.str(),
      cv::putText(img,
                  std::to_string(d.id) + "|" + ss.str() + "|" +
                      std::to_string(d.category_id),
                  cv::Point2i(tl.x, std::max(5, tl.y - 5)),
                  cv::FONT_HERSHEY_DUPLEX, 0.55, cv::Scalar(255, 255, 255), 1,
                  false);
    }
  }
  return img;
...
void draw_ellipse_dashed(cv::Mat img, const Ellipse &ell,
  ...
  // Safety check
  if (std::isnan(c[0]) || std::isnan(c[1]) || std::isnan(axes[0]) ||
      std::isnan(axes[1]) || std::isinf(c[0]) || std::isinf(c[1]) ||
      std::isinf(axes[0]) || std::isinf(axes[1]) || axes[0] <= 0 ||
      axes[1] <= 0)
    return;
...
cv::Mat FrameDrawer::DrawProjections(cv::Mat img) {
  std::vector<ObjectProjectionWidget,
              Eigen::aligned_allocator<ObjectProjectionWidget>>
      projections;
  {
    unique_lock<mutex> lock(mMutex);
  ...
  for (auto w : projections) {
    ...
    if (w.in_map) {
      try {
        cv::ellipse(img, cv::Point2f(c[0], c[1]), cv::Size2f(axes[0], axes[1]),
                    TO_DEG(angle), 0, 360, color, 2);
      } catch (...) {
      }
      // stringstream ss;
      // ss << std::fixed << std::setprecision(3) << w.uncertainty;
      // cv::putText(img, ss.str(), cv::Point2f(c[0], c[1]),
      // cv::FONT_HERSHEY_DUPLEX, 0.55, cv::Scalar(255, 255, 255), 1, false);
    } else
      draw_ellipse_dashed(img, ell, color, 2);
    // cv::rectangle(img, cv::Point2i(bb[0], bb[1]), cv::Point2i(bb[2], bb[3]),
    // color, 2);
  }
