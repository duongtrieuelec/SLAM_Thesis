...
System::System(const string &strVocFile, const string &strSettingsFile,
               ...
               const bool bUseARViewer, int use_objects_in_local_BA)
    : mSensor(sensor), mpViewer(static_cast<Viewer *>(NULL)), mbReset(false),
      mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false),
      mShouldQuit(false) {
  // Output welcome message
  cout << endl
       << "ORB-SLAM2 Copyright (C) 2022 Matthieu Zins, Inria, LORIA, Université "
          "de Lorraine;"
       << endl
  ...
  mpLocalMapper =
      new LocalMapping(mpMap, mSensor == MONOCULAR, use_objects_in_local_BA);
  ...
  if (use_objects_in_local_BA <= 1) {
    ...
    local_object_mapping_ =
        new thread(&ORB_SLAM2::LocalObjectMapping::Run, local_object_mapper_);
  }
  ...
  if (bUseARViewer) {
    mpARViewer =
        new ARViewer(this, mpFrameDrawer, mpMap, mpTracker, strSettingsFile);
    mptARViewer = new thread(&ARViewer::Run, mpARViewer);
    mpTracker->SetARViewer(mpARViewer);
    // mpARViewer->AddModel("/home/mzins/dev/ORB_SLAM2_obj/Examples/Monocular/AR/banana_2.obj",
...
// TrackRGBD with object detections
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap,
                          const double &timestamp,
  ...
  // Check if need pause
  if (mpViewer) {
    while (mpViewer->isPaused() && !mpViewer->isStopped() &&
           !mpViewer->isFinished()) {
      usleep(90000);
    }
  }
  ...
  // Use GrabImageRGBD with detections support
  cv::Mat Tcw = mpTracker->GrabImageRGBD(im, depthmap, timestamp, detections);
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp,
  ...
  cv::Mat Tcw = mpTracker->GrabImageMonocular(im, timestamp, detections,
                                              force_relocalize);
...
// TrackMonocular with person segmentation mask
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp,
                               const std::vector<Detection::Ptr> &detections,
                               bool force_relocalize,
...
void System::Shutdown() {
  ...
  if (local_object_mapper_)
    local_object_mapper_->RequestFinish();
  if (mpARViewer)
    mpARViewer->RequestFinish();
  if (mpViewer) {
  ...
  // Wait unitl AR viewer is stopped
  if (mpARViewer) {
    while (!mpARViewer->isFinished()) {
      usleep(5000);
    }
  }
...
void System::SaveKeyFrameTrajectoryJSON(
    const string &filename, const std::vector<std::string> &frames_filenames) {
  vector<KeyFrame *> vpKFs = mpMap->GetAllKeyFrames();
  sort(vpKFs.begin(), vpKFs.end(), KeyFrame::lId);
...
void System::SaveMapObjectsOBJ(const string &filename) {
  ...
  for (size_t i = 0; i < objects.size(); ++i) {
    ...
    for (int j = 0; j < pts.rows(); ++j) {
      f << "v " << setprecision(7) << " " << pts(j, 0) << " " << pts(j, 1)
        << " " << pts(j, 2) << " " << c[2] << " " << c[1] << " " << c[0]
        << endl;
    }
  }
...
void System::remove_nth_object_by_cat(int cat, int nth) {
  ...
  for (auto o : objects) {
    if (o->GetTrack()->GetCategoryId() == cat) {
      std::cout << "Object " << o->GetTrack()->GetId() << " is from  cat "
                << o->GetTrack()->GetCategoryId() << "\n";
      ++i;
      if (i == nth) {
