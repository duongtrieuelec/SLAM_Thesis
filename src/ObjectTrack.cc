...
int chair_count = 0;
ObjectTrack::Ptr ObjectTrack::CreateNewObjectTrack(
    unsigned int cat, const BBox2 &bbox, double score, const Matrix34d &Rt,
  ...
  obj->unc_ =
      0.5; // ===> depends on the variance of the reconstruction gaussian curve
  return obj;
}
...
bool ObjectTrack::ReconstructFromLandmarks(Map *map) {
  ...
  auto [status, ellipsoid] =
      ReconstructEllipsoidFromLandmarks(bboxes, Rts, tracker_->GetK(), pts);
...
bool ObjectTrack::ReconstructCrocco(bool use_two_passes) {
  if (this->GetAngularDifference() < TO_RAD(10.0)) {
    // std::cerr << "Impossible to triangulate the center: not enough angular
    // difference.\n";
    return false;
  }
  ...
  auto [status, ellipsoid] =
      ReconstructEllipsoidCrocco(bboxes, Rts, tracker_->GetK(), use_two_passes);
...
bool ObjectTrack::ReconstructFromCenter(bool use_keyframes) {
  double ang_diff = this->GetAngularDifference();
  if (ang_diff < TO_RAD(10.0)) {
    std::cerr << "Impossible to triangulate the center: not enough angular "
                 "difference. Current: "
              << ang_diff * 180.0 / M_PI << " deg.\n";
    return false;
  }
  ...
  auto [status, ellipsoid] =
      ReconstructEllipsoidFromCenters(bboxes, Rts, tracker_->GetK());
...
void ObjectTrack::AddDetection(const BBox2 &bbox, double score,
                               const Matrix34d &Rt, unsigned int frame_idx,
                               KeyFrame *kf) {
...
std::tuple<std::vector<BBox2, Eigen::aligned_allocator<BBox2>>,
...
ObjectTrack::CopyDetectionsInKeyFrames() {
  ...
  if (map_object_) {
    ...
    for (auto &it : keyframes_bboxes_) {
      ...
      poses.push_back(
          cvToEigenMatrix<double, float, 3, 4>(it.first->GetPose()));
...
void ObjectTrack::OptimizeReconstruction(Map *map) {
  if (!map_object_) {
    // std::cerr << "Impossible to optimize ellipsoid. It first requires a "
    //              "initial reconstruction." << std::endl;
    return;
  }
  const Ellipsoid &ellipsoid = map_object_->GetEllipsoid();
  const Eigen::Matrix3d &K = tracker_->GetK();

  // std::cout << "ENTERING OptimizeReconstruction id=" << id_ << std::endl;
  // std::cout << "===============================> Start ellipsoid optimization
  // " << id_ << std::endl;
  typedef g2o::BlockSolver<g2o::BlockSolverTraits<6, 1>> BlockSolver_6_1;
  BlockSolver_6_1::LinearSolverType *linear_solver =
      new g2o::LinearSolverDense<BlockSolver_6_1::PoseMatrixType>();
  ...
  if (!optimizer.addVertex(vertex)) {
    // std::cerr << "FAILED TO ADD VERTEX!" << std::endl;
    return;
  }
  ...
  int edges_added = 0;
  for (size_t i = 0; i < bboxes_.size() && it_bb != bboxes_.end() &&
                     it_Rt != Rts_.end() && it_s != scores_.end();
       ++i, ++it_bb, ++it_Rt, ++it_s) {
    Eigen::Matrix<double, 3, 4> P = K * (*it_Rt);
    EdgeEllipsoidProjection *edge = new EdgeEllipsoidProjection(
        P, Ellipse::FromBbox(*it_bb), ellipsoid.GetOrientation());
    edge->setId(i);
    ...
    if (optimizer.addEdge(edge)) {
      ...
      // std::cerr << "Failed to add edge " << i << std::endl;
    }
  }

  // std::cout << "Edges Added: " << edges_added << " | Optimizer Edges: " <<
  // optimizer.edges().size() << std::endl;

  if (optimizer.edges().size() == 0) {
    // std::cerr << "No edges in optimizer! Aborting." << std::endl;
    return;
  }

  if (!optimizer.initializeOptimization()) {
    // std::cerr << "Optimization initialization failed! (Ret False)" <<
    // std::endl;
    return;
  }

  if (optimizer.edges().size() == 0) {
    // std::cerr << "Optimizer has 0 edges after initialization. Skipping."
    //           << std::endl;
    return;
  }
...
bool ObjectTrack::ReconstructFromSamplesEllipsoid() {
  ...
  int best_idx =
      std::distance(accu.begin(), std::max_element(accu.begin(), accu.end()));
...
bool ObjectTrack::ReconstructFromSamplesCenter() {
  ...
  int best_idx =
      std::distance(accu.begin(), std::min_element(accu.begin(), accu.end()));
  Eigen::Vector3d center = samples[best_idx];
  double d = (Rt0 * center.homogeneous()).z();
  mean_size *= d;
  Ellipsoid ellipsoid(Eigen::Vector3d(mean_size, mean_size, mean_size),
                      Eigen::Matrix3d::Identity(), center);
...
void ObjectTrack::TryResetEllipsoidFromMaPoints() {
  ...
  double size_ratio = std::min(std::min(axes[0], axes[1]), axes[2]) /
                      std::max(std::max(axes[0], axes[1]), axes[2]);
  ...
  /*else {
      ...
      for (auto* p : points)
      {
          ...
          Eigen::Vector3d
  pt(pos.at<float>(0),pos.at<float>(1),pos.at<float>(2)); if (ell.IsInside(pt))
  {
...
//     if (!map_object_) {
//         std::cerr << "Impossible to optimize ellipsoid. It first requires a
//         initial reconstruction." << std::endl; return ;
//     }
//     const Ellipsoid& ellipsoid = map_object_->GetEllipsoid();
//     const Eigen::Matrix3d& K = tracker_->GetK();
...
//             new ceres::NumericDiffCostFunction<EllipsoidProjectionError,
//             ceres::CENTRAL, 1, 6>(
//                 new EllipsoidProjectionError(bboxes_[i], P, orientation,
//                 axes)
//                 // new EllipsoidProjectionError(bb_center, P, orientation,
//                 axes)
//             ), nullptr, params
//         );
//     }
...
//     v0 << params[0], params[1], params[2], params[3], params[4], params[5],
...
//     Ellipsoid new_ellipsoid(Eigen::Vector3d(params[0], params[1], params[2]),
//     orientation, Eigen::Vector3d(params[3], params[4], params[5]));
//     map_object_->SetEllipsoid(new_ellipsoid);
