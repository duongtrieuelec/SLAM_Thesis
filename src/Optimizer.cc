  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin();
      ...
      if (pKF->mvuRight[mit->second] < 0) {
        ...
        float weight = pKF->mvFeatureWeights[mit->second];
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (pMP) {
        ...
        if (pFrame->mvuRight[i] < 0) {
          ...
          float weight = pFrame->mvFeatureWeights[i];
          e->setInformation(Eigen::Matrix2d::Identity() * invSigma2 * weight);
        ...
        } else // Stereo observation
        {
          ...
          float weight = pFrame->mvFeatureWeights[i];
          Eigen::Matrix3d Info =
              Eigen::Matrix3d::Identity() * invSigma2 * weight;
          e->setInformation(Info);
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag,
  ...
  if (use_objects == 1) {
    for (MapObject *pObj : objects) {
      ...
      for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin();
        ...
        auto [kf_bboxes, kf_scores] =
            pObj->GetTrack()->CopyDetectionsMapInKeyFrames();
        ...
        if (kf_bboxes.find(kf) != kf_bboxes.end()) {
          ...
          EdgeSE3ProjectEllipsoidOnlyPose *e =
              new EdgeSE3ProjectEllipsoidOnlyPose(ell, ellipsoid, K);
          e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                              optimizer.vertex(kf->mnId)));
          Eigen::Matrix<double, 1, 1> information_matrix =
              Eigen::Matrix<double, 1, 1>::Identity();
          e->setInformation(information_matrix);
          // std::cout << "Added edge with ellipsoid " << tr->GetId() <<
          // std::endl;
          optimizer.addEdge(e);
        }
      }
    ...
    for (MapObject *pObj : objects) {
      ...
      EllipsoidQuat ellipsoid_quat =
          EllipsoidQuat::FromEllipsoid(pObj->GetEllipsoid());
      ...
      for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin();
        ...
        if (kf_bboxes.find(kf) != kf_bboxes.end()) {
          ...
          // EdgeSE3ProjectEllipsoidCenter *e = new
          // EdgeSE3ProjectEllipsoidCenter(ell, K);
          e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                              optimizer.vertex(id)));
          e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                              optimizer.vertex(kf->mnId)));
  ...
  if (use_objects == 2) {
    for (MapObject *pObj : objects) {
      ...
      int id = maxMPid + 1 + pObj->GetTrack()->GetId();
      VertexEllipsoidQuat *vEll =
          static_cast<VertexEllipsoidQuat *>(optimizer.vertex(id));
      EllipsoidQuat ell_quat_est = vEll->estimate();
      Ellipsoid new_ellipsoid = ell_quat_est.ToEllipsoid();
      pObj->SetEllipsoid(new_ellipsoid);

      // g2o::VertexSBAPointXYZ* vEll =
      // static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(id));
