...
Eigen::Vector3d TriangulatePoints2(const Eigen::Vector2d &uv1,
...
Eigen::Vector3d TriangulatePoints(
    const std::vector<Eigen::Vector2d,
                      Eigen::aligned_allocator<Eigen::Vector2d>> &points,
    const std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>>
        &projections) {
  const int n = projections.size();

  // Protection: need at least 2 projections to triangulate
  if (n < 2 || points.size() < 2 || points.size() != static_cast<size_t>(n)) {
...
Eigen::Vector3d TriangulatePointsRansac(
    const std::vector<Eigen::Vector2d,
                      Eigen::aligned_allocator<Eigen::Vector2d>> &points,
    const std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>>
        &projections,
  ...
  for (int k = 0; k < limits; ++k) {
    ...
    int i0 = pair.first;
    int i1 = pair.second;
    Eigen::Vector3d result = TriangulatePoints2(
        points[i0], points[i1], projections[i0], projections[i1]);
  ...
  std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>>
      inliers_projections(max_nb_inliers);
  std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>>
      inliers_points(max_nb_inliers);
  a = 0;
  for (int i = 0; i < n; ++i) {
...
std::pair<bool, Ellipsoid> ReconstructEllipsoidFromCenters(
    const std::vector<BBox2, Eigen::aligned_allocator<BBox2>> &bboxes,
  ...
  // Eigen::Vector3d center_no_ransac = TriangulatePoints(points2d,
  // projections); Eigen::Vector3d center = TriangulatePointsRansac(points2d,
  // projections, 1000);
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if ((points2d[i] - Eigen::Vector2d(u, v)).norm() > 100) {
      // std::cerr << "Reconstruction failed: reconstructed center is too far "
      //            "from a detection" << std::endl;
      return {false, Ellipsoid()};
    }
  ...
  return {true, Ellipsoid(Eigen::Vector3d::Ones() * mean_3D_size * 0.5,
                          Eigen::Matrix3d::Identity(), center)};
}

std::pair<bool, Ellipsoid> ReconstructEllipsoidFromLandmarks(
    const std::vector<BBox2, Eigen::aligned_allocator<BBox2>> &bboxes,
    const std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>> &Rts,
    const Eigen::Matrix3d &K,
  ...
  for (size_t i = 0; i < bboxes.size(); ++i) {
    Eigen::Matrix<double, 3, Eigen::Dynamic> uvs =
        K * ((Rts[i].block<3, 3>(0, 0) * pts).colwise() + Rts[i].col(3));
...
Eigen::MatrixXd compute_B_perspective(const Eigen::Matrix<double, 3, 4> &P) {
  ...
  double p11_2 = p11 * p11, p12_2 = p12 * p12, p13_2 = p13 * p13,
         p14_2 = p14 * p14;
  double p21_2 = p21 * p21, p22_2 = p22 * p22, p23_2 = p23 * p23,
         p24_2 = p24 * p24;
  double p31_2 = p31 * p31, p32_2 = p32 * p32, p33_2 = p33 * p33,
         p34_2 = p34 * p34;
  Eigen::MatrixXd B(6, 10);
  B << p11_2, 2 * p12 * p11, 2 * p13 * p11, 2 * p14 * p11, p12_2, 2 * p13 * p12,
      2 * p14 * p12, p13_2, 2 * p13 * p14, p14_2, p21 * p11,
      p21 * p12 + p22 * p11, p23 * p11 + p21 * p13, p24 * p11 + p21 * p14,
      p22 * p12, p22 * p13 + p23 * p12, p22 * p14 + p24 * p12, p23 * p13,
      p23 * p14 + p24 * p13, p24 * p14, p31 * p11, p31 * p12 + p32 * p11,
      p33 * p11 + p31 * p13, p34 * p11 + p31 * p14, p32 * p12,
      p32 * p13 + p33 * p12, p32 * p14 + p34 * p12, p33 * p13,
      p33 * p14 + p34 * p13, p34 * p14, p21_2, 2 * p22 * p21, 2 * p23 * p21,
      2 * p24 * p21, p22_2, 2 * p23 * p22, 2 * p24 * p22, p23_2, 2 * p23 * p24,
      p24_2, p31 * p21, p31 * p22 + p32 * p21, p33 * p21 + p31 * p23,
      p34 * p21 + p31 * p24, p32 * p22, p32 * p23 + p33 * p22,
      p32 * p24 + p34 * p22, p33 * p23, p33 * p24 + p34 * p23, p34 * p24, p31_2,
      2 * p32 * p31, 2 * p33 * p31, 2 * p34 * p31, p32_2, 2 * p33 * p32,
      2 * p34 * p32, p33_2, 2 * p33 * p34, p34_2;
  return B;
}

Ellipsoid reconstruct_ellipsoid_lstsq(
    const std::vector<Ellipse, Eigen::aligned_allocator<Ellipse>> &ellipses,
    const std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>>
        &projections) {
  ...
  for (int index = 0; index < n_views; ++index) {
    ...
    Eigen::Matrix<double, 3, 4> new_P =
        (projections[index].transpose() * H_inv.transpose()).transpose();
...
std::pair<bool, Ellipsoid> ReconstructEllipsoidCrocco(
    const std::vector<BBox2, Eigen::aligned_allocator<BBox2>> &bboxes,
    const std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>> &Rts,
  ...
  return {true, Ellipsoid(ellipsoid.GetAxes(), ellipsoid.GetOrientation(),
                          ellipsoid.GetCenter())};
}
