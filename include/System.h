...
public:
  ...
  System(const string &strVocFile, const string &strSettingsFile,
         ...
         const bool bUseARViewer = false, int use_objects_in_local_BA = 0);
  ...
  // Overload with person segmentation mask for keypoint filtering
  cv::Mat TrackMonocular(const cv::Mat &im, const double &timestamp,
                         const std::vector<Detection::Ptr> &detections,
                         bool force_relocalize, const cv::Mat &person_mask);
  ...
  void
  SaveKeyFrameTrajectoryJSON(const string &filename,
  ...
  void SaveMapPointsOBJ(const string &filename);

  bool ShouldQuit() const { return mShouldQuit; }

  void SaveMapObjectsOBJ(const string &filename);
  void SaveMapObjectsTXT(const std::string &filename);

  void SetRelocalizationMode(enumRelocalizationMode mode) {
    reloc_mode_ = mode;
  }

  enumRelocalizationMode GetRelocalizationMode() const { return reloc_mode_; }

  // remove the n-th object of the category cat from the map
  void remove_nth_object_by_cat(int cat, int nth);
