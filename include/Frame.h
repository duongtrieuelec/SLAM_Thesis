...
public:
  ...
  // Filter out keypoints that fall inside any of the given bounding boxes
  // bbox format: [x1, y1, x2, y2] (Eigen::Vector4d)
  void FilterKeypointsInBboxes(const std::vector<Eigen::Vector4d> &bboxes);

  // Filter out keypoints where the mask has non-zero value (for
  // segmentation-based filtering)
  void FilterKeypointsWithMask(const cv::Mat &mask);

  // Assign weights to keypoints based on a float mask
  void AssignFeatureWeights(const cv::Mat &weight_mask);
...
public:
  ...
  // Vector of feature weights (default 1.0)
  std::vector<float> mvFeatureWeights;
