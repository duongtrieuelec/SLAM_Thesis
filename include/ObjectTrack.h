...
enum class ObjectTrackStatus { ONLY_2D, INITIALIZED, IN_MAP, BAD };
...
public:
  ...
  static ObjectTrack::Ptr CreateNewObjectTrack(unsigned int cat,
                                               const BBox2 &bbox, double score,
                                               const Matrix34d &Rt,
                                               unsigned int frame_idx,
                                               Tracking *tracker, KeyFrame *kf);
  ...
  void AddDetection(const BBox2 &bbox, double score, const Matrix34d &Rt,
                    unsigned int frame_idx, KeyFrame *kf);

  unsigned int GetCategoryId() const { return category_id_; }
  unsigned int GetId() const { return id_; }

  // ------- For osmap ------
  void SetId(unsigned int id) { id_ = id; }

  void SetLastObsFrameId(int frame_id) { last_obs_frame_id_ = frame_id; }

  void SetColor(const cv::Scalar &color) { color_ = color; }

  void SetStatus(ObjectTrackStatus status) { status_ = status; }
  ...
  BBox2 GetLastBbox() const { return bboxes_.front(); }

  size_t GetLastObsFrameId() const { return last_obs_frame_id_; }

  size_t GetNbObservations() const { return bboxes_.size(); }
  size_t GetNbObservationsInKeyFrame() const {
    return keyframes_bboxes_.size();
  }

  cv::Scalar GetColor() const { return color_; }
  MapObject *GetMapObject() { return map_object_.get(); }
  ...
  double GetLastObsScore() const { return last_obs_score_; }

  std::pair<
      std::unordered_map<
          KeyFrame *, BBox2, std::hash<KeyFrame *>, std::equal_to<KeyFrame *>,
  ...
  std::unordered_map<MapPoint *, int>
  GetFilteredAssociatedMapPoints(int threshold) const {
    std::unique_lock<std::mutex> lock(mutex_associated_map_points_);
    int limit =
        threshold; //  std::max(10.0, threshold * keyframes_bboxes_.size());
...
public:
  ...
  RingBuffer<BBox2, Eigen::aligned_allocator<BBox2>> bboxes_ =
      RingBuffer<BBox2, Eigen::aligned_allocator<BBox2>>(max_frames_history);
  RingBuffer<Matrix34d, Eigen::aligned_allocator<Matrix34d>> Rts_ =
      RingBuffer<Matrix34d, Eigen::aligned_allocator<Matrix34d>>(
          max_frames_history);
